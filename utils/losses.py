...
class Borji_auc_loss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        for rep in range(n_rep):
            ...
            S_neg = tf.SparseTensor(indices=s_neg[:, rep], values=[1] * n_fix,
                                    dense_shape=[n_fix])  # CB. from index to binary mask
...
def borji_auc_loss_fn_wrong(y_true, y_pred):
    ...
    n_pixels = tf.cast(tf.shape(S)[0], tf.int64)
    s_neg = tf.experimental.numpy.random.randint(0, n_pixels, size=[n_fix, n_rep])  # negative samples
    S_rand = tf.gather(tf.reshape(saliency_map, (8, -1)),
                       s_neg)  # Saliency map values at random locations (including fixated locations!? underestimated)
