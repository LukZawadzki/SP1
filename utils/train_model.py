...
from utils.autoencoder import create_autoencoder
from config import *
...
def train_model(model_name, x_path, y_path, path=None):
    x_train, x_valid = (tf.keras.preprocessing.image_dataset_from_directory(
        ...
        batch_size=16, validation_split=0.25, subset="both", seed=57))
    ...
    x_train = x_train.map(tf.keras.applications.vgg19.preprocess_input)
    x_valid = x_valid.map(tf.keras.applications.vgg19.preprocess_input)
    ...
    y_train, y_valid = tf.keras.preprocessing.image_dataset_from_directory(
        ...
        color_mode="grayscale", validation_split=0.25, subset="both", seed=57)
    ...
    model.fit(train_set, validation_data=valid_set, epochs=20, callbacks=[saving_models])
